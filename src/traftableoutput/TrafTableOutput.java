 ...
 * @since 6-apr-2003
 */
public class TrafTableOutput extends BaseStep implements StepInterface {
	private static Class<?> PKG = TrafTableOutputMeta.class; // for i18n
																// purposes,
																// needed by
																// Translator2!!

	private TrafTableOutputMeta meta;
	private TrafTableOutputData data;

	public TrafTableOutput(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
			Trans trans) {
		super(stepMeta, stepDataInterface, copyNr, transMeta, trans);
	}

	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
		meta = (TrafTableOutputMeta) smi;
		data = (TrafTableOutputData) sdi;
			...
			if (!meta.specifyFields()) {
				...
				for (int i = 0; i < meta.getFieldDatabase().length; i++) {
					...
					if (data.valuenrs[i] < 0) {
						throw new KettleStepException(BaseMessages.getString(PKG, "TableOutput.Exception.FieldRequired",
	...
	protected Object[] writeToTable(RowMetaInterface rowMeta, Object[] r) throws KettleException {
		...
		if (insertStatement == null) {
			String sql = data.db.getInsertStatement(environmentSubstitute(meta.getSchemaName()), tableName,
					data.insertRowMeta);
			if (log.isDetailed()) {
		...
		try {
			...
			if ((data.commitSize > 0) && ((commitCounter % data.commitSize) == 0)) {
				if (data.db.getUseBatchInsert(data.batchMode)) {
					try {
						...
						throw TrafDatabase.createKettleDatabaseBatchException("Error updating batch", ex);
					} catch (Exception ex) {
			...
			if (meta.isReturningGeneratedKeys()) {
				...
				if (extraKeys.getRowMeta().size() > 0) {
					...
					// we have to throw something here, else we don't know what
					// the
					// type is of the returned key(s) and we would violate our
					// own rule
	...
	public boolean init(StepMetaInterface smi, StepDataInterface sdi) {
		...
		if (super.init(smi, sdi)) {
			try {
				...
				//
				data.useSafePoints = data.databaseMeta.getDatabaseInterface().useSafePoints()
						&& getStepMeta().isDoingErrorHandling();

				// Get the boolean that indicates whether or not we can/should
				// release
				...
				// - if you are running the transformation as a single database
				// transaction (unique connections)
				// - if we are reverting to save-points
				//
				data.batchMode = meta.useBatchUpdate() && data.commitSize > 0 && !meta.isReturningGeneratedKeys()
						&& !getTransMeta().isUsingUniqueConnections() && !data.useSafePoints;

				// Per PDI-6211 : give a warning that batch mode operation in
				// combination with step error handling can lead to
				...
				data.db = new TrafDatabase(this, meta.getDatabaseMeta());
				...
				if (!meta.isPartitioningEnabled() && !meta.isTableNameInField()) {
					...
					// copy
					//
					if (meta.truncateTable() && ((getCopy() == 0 && getUniqueStepNrAcrossSlaves() == 0)
	 ...
	 * @return
	 */
	protected TrafTableOutputMeta getMeta() {
		return meta;
	}
	 ...
	 * @return
	 */
	protected TrafTableOutputData getData() {
		return data;
	}

	protected void setMeta(TrafTableOutputMeta meta) {
		this.meta = meta;
	}

	protected void setData(TrafTableOutputData data) {
		this.data = data;
	}
}
